# Analysis microbial communities from DNA (metagenomic shotgun sequencing) data processed using phyloFlash.
...
all_paths <-list.files(path = "path/to/phyloFlash/DNA/NTUabundance/files", pattern = "*.csv",full.names = TRUE)
...
DNA<-phyloseq(NTU,TAX,SAMPLE_DATA)


# fix taxa table to replace unknowns etc. (using microViz package)
DNA_fixed<-tax_fix(DNA, min_length = 4)
...
DNA_fixed<-subset_taxa(DNA_fixed,genus != "Unknown Family family")


# Save phyloseq objects for posterity
saveRDS(DNA, here("DNA_ps.rds"))
saveRDS(DNA_fixed, here("DNA_fixed_ps.rds"))